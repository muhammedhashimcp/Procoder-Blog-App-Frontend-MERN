						...
						<div className="flex flex-wrap -mx-3">
							<div className="mb-8 lg:mb-0 w-full lg:w-1/4 px-3">
								<div className="py-4 px-6 bg-gray-600 shadow rounded">
									...
									<ul>
										{catLoading ? <LoadingComponent /> : catAppErr || catServerErr ? <h1>{catServerErr} {catAppErr}</h1> : categoryList?.length <= 0 ? <h1 className="text-yellow-400 text-lg text-center">No Category Found</h1> : categoryList?.map((category) => (<li key={category._id}>
							...
							<div className="w-full lg:w-3/4 px-3">
								{/* Post goes here */}
								{ appErr || serverErr ? <h1 className="text-yellow-600 text-center text-lg">Err</h1> : postLists?.length <= 0 ? <h1 className="text-yellow-400 text-lg text-center">No Post Found</h1> : postLists?.map((post) => (<div key={post._id} className="flex flex-wrap bg-gray-900 -mx-3  lg:mb-6">
